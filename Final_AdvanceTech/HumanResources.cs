...
namespace Final_AdvanceTech
{
    public partial class HumanResources : Form
    {
        ...
        {
            if (e.RowIndex >= 0) // Kiểm tra nếu người dùng nhấp vào một dòng hợp lệ
            {
                // Lấy thông tin từ các ô của dòng đã chọn
                int employeeID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["EmployeeID"].Value);
                int month = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["Month"].Value);
                int year = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["Year"].Value);
                decimal score = Convert.ToDecimal(dataGridView1.Rows[e.RowIndex].Cells["Score"].Value);
                string comments = dataGridView1.Rows[e.RowIndex].Cells["Comments"].Value.ToString();

                // Hiển thị hoặc xử lý thông tin đã chọn
                MessageBox.Show($"Thông tin hiệu suất:\nMã nhân viên: {employeeID}\nTháng: {month}\nNăm: {year}\n" +
                                 $"Điểm: {score}\nNhận xét: {comments}");
            }
        }
